function set_itog(){
    ...
    var net_itog = $('#net_itog').text();
    var vat_cost = net_itog*parseFloat(vat)/100;
    vat_cost = vat_cost.toFixed(2);
    $('#net_vat').text(vat_cost);
    var total = parseFloat(net_itog)+parseFloat(vat_cost);
    total = total.toFixed(2);
    $('#total_itog').text(total);
//     alert('net itog='+net_itog+' total='+total);
    return false;
}

function set_itog_old( is_add, count_items,income){
    if( !is_add ) is_add = 0;
